...
[cite_start]这四个域是系统的灵魂，决定了代码的解耦边界 [cite: 1]：
...
1. 由于会进行缓存预热，静态数据都会搬进高速缓存，所以部分静态数据可以使用多个表结构进行join查询，增加可读性和可维护性，性能再差反正只会在预热时加载一次，影响近乎为0
2. 乐观锁@Version
3. 课程tag只存子节点，通过表映射路径及优先级
...
10. BitMap
11. MQ，类型未定，看看到时候学什么
    目前阻塞队列使用Java内部的够用，数据库有本地消息补偿表，暂时不考虑引入
12. Swagger
13. Slf4j
